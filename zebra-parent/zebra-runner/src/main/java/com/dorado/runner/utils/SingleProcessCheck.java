...
public enum SingleProcessCheck {
	...
	UserReport(1, "for test"), // for test
	MentorIndexMerger(10, "Mentor index merger"), // 导师搜索，定期将增量索引合并到主索引Runner
	UserIndexMergerDaemon(11, "User index merger"), // 导师搜索，定期将增量索引合并到主索引Runner
	;
	 ...
	 * @param name
	 */
	private ProcessType(int port, String name) {
	    this.port = BASE + port;
	    this.name = name;
	}
