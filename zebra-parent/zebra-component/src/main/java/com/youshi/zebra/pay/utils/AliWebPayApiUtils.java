 ...
 * @date 2017年2月4日
 */
public class AliWebPayApiUtils {
	private static final Logger logger = LoggerFactory.getLogger(AliWebPayConstants.LOGGER_NAME); 
	
	private static final AlipayClient client = new DefaultAlipayClient(AliWebPayConstants.ALIPAY_GATEWAY, AliWebPayConstants.APP_ID, 
			AliWebPayConstants.APP_PRIVATE_KEY, AliWebPayConstants.FORMAT, AliWebPayConstants.CHARSET, 
			AliWebPayConstants.ALIPAY_PUBLIC_KEY, AliWebPayConstants.SIGN_TYPE);
	...
	public static final boolean verifyNotify(Map<String, String> paramsMap) {
		return verifyNotify(paramsMap, AliWebPayConstants.ALIPAY_PUBLIC_KEY);
	}
	...
	public static final boolean verifyNotify(Map<String, String> paramsMap, String aliPublicKey) {
		...
		try {
			...
			succ = AlipaySignature.rsa256CheckContent(AlipaySignature.getSignCheckContentV1(paramsMap), sign,
					aliPublicKey, AliWebPayConstants.CHARSET);
	...
	public static boolean verify(Map<String, String> paramsMap) {
		...
		try {
			signVerified = AlipaySignature.rsaCheckV1(paramsMap, 
					AliWebPayConstants.ALIPAY_PUBLIC_KEY, AliWebPayConstants.CHARSET);
		} catch (AlipayApiException e) {
			AlipayCallException ex = dealAlipayApiException(e);
			throw ex;
		}
